...
from reward_shaping.lti_filtering.offline_discrete_time_python_monitor import \
    MTLFilteringOfflineDiscreteTimePythonMonitor


class MTLFilteringOfflineEvaluator(STLVisitor):
    def __init__(self, spec):
        self.spec = spec
        generator = None

        assert self.spec.language == Language.PYTHON, f"not implemented {self.spec.language}"
        assert self.spec.time_interpretation == TimeInterpretation.DISCRETE, f"not implemented {self.spec.time_interpretation}"
        generator = MTLFilteringOfflineDiscreteTimePythonMonitor()
