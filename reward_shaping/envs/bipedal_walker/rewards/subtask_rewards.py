...
class ContinuousFalldownReward(RewardFunction):
    ...
    def __call__(self, state, action=None, next_state=None, info=None) -> float:
        ...
        lidar = min(state[-10:])
        return info['dist_hull_limit'] - lidar
...
class ContinuousVerticalSpeedReward(RewardFunction):
    """
    always(abs(v_y) <= speed_y_hull_limit
    state[3] = 0.3 * vel.y * (VIEWPORT_H / SCALE) / FPS,  #also normalized
    """
    def __call__(self, state, action=None, next_state=None, info=None) -> float:
        assert 'speed_y_hull_limit' in info
        return info['speed_y_hull_limit'] - abs(state[3])
...
class ContinuousHullAngleVelocityReward(RewardFunction):
    ...
    """
    def __call__(self, state, action=None, next_state=None, info=None) -> float:
        assert 'angle_vel_hull_limit' in info
        phi_dot = state[1]
        return info['angle_vel_hull_limit'] - abs(phi_dot)
